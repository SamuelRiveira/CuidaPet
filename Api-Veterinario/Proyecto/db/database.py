...
class Database:
    ...
    def execute_query(query, params=None, fetch=True):
        """
        Execute a parameterized SQL query and return results as a list of dictionaries.

        Args:
            query (str): SQL query with potential %s placeholders
            params (tuple): Parameters for the query
            fetch (bool): Indicates if results should be fetched (True) or just executed (False)

        Returns:
            list: List of dictionaries with results, or None if fetch=False
        """
        try:
    ...
    def insert(table, data, returning=None):
        """Insert data into a table and optionally return specified columns."""
